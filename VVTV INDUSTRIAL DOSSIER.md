...
O **Bloco VII** define o coração econômico e inteligente do VoulezVous.TV: como o sistema transforma cada minuto transmitido em valor mensurável, auditável e recorrente, enquanto adapta sua programação através de algoritmos de desejo computável e inteligência artificial.
...
### 0.1 Arquitetura de Programação Adaptativa
...
### 0.2 Configuração de Negócio (business_logic.yaml)

O sistema é controlado por um arquivo de configuração YAML que define todos os parâmetros de negócio:
...
### 7.1 Implementação Técnica: Business Logic Engine

#### Arquitetura de Seleção Determinística

O sistema utiliza **Gumbel-Top-K sampling** com seeds determinísticos para garantir reprodutibilidade auditável:
...
let ordered = match method {
    SelectionMethod::GumbelTopK => {
        ...
        let scaled_scores: Vec<f64> = ordered.iter()
            .map(|(_, score, _)| *score / temperature).collect();
...
#### Configuração via Cartão YAML

O **Cartão Perfurado do Dono** (`business_logic.yaml`) controla todos os parâmetros:
...
O sistema integra LLMs como **conselheiros** (5% do trabalho), não como controladores:
...
O **Curator Vigilante** monitora sinais estéticos e aplica correções quando necessário:
...
**Sinais monitorados:**
- Duplicação de tags
- Desvio de temperatura de cor
- Saturação excessiva
- Ritmo inadequado
- Violação de limites LUFS

#### Token Bucket & Rate Limiting

O sistema usa **token bucket** para controlar intervenções do Curator:

- `token_bucket_capacity = 6`
- `token_bucket_refill_per_hour = 6`
...
# Validar novo cartão
vvtvctl business-logic validate configs/new_business_logic.yaml

# Recarregar configuração (hot reload)
vvtvctl business-logic reload

# Exportar para automação
vvtvctl business-logic show --format json
```

#### Observabilidade & Auditoria

Todos os eventos são logados estruturadamente:

```json
{
  "timestamp": "2025-10-13T14:30:00Z",
  "event": "planner.selection",
  "seed": 12345678,
  "temperature": 0.85,
  "top_k": 12,
  "indices": [3, 7, 1, 9, 2],
  "scores_norm": [0.92, 0.87, 0.83, 0.79, 0.75],
  "llm_action": {
    "source": "curator_vigilante",
    "mode": "apply",
    "reason": "aesthetic_correction",
    "confidence": 0.73
  }
}
```
...
1. **BusinessLogic::load_from_file** valida o cartão YAML
2. **Planner::run_once_async** pontua candidatos e aplica bias
3. **LLM hooks** fornecem sugestões (com circuit breaker)
4. **Gumbel-Top-K** executa seleção determinística
5. **CuratorVigilante** revisa e reordena se necessário
6. **SqlitePlanStore** persiste decisões auditáveis
7. **Realizer** continua o fluxo T-4h
...
- `BusinessLogic::load_from_file` converte YAML em tipos Rust com validação
- Repositório inclui cartão exemplo em `configs/business_logic.yaml`
- CLI `vvtvctl business-logic show|validate|reload` para operação
- Estrutura mantém restrições operacionais (bias máximo, epsilon em [0,1])

**B) Seleção determinística (Gumbel-Top-k + seed por slot)**
- Módulo `vvtv-core/src/plan/selection/mod.rs` implementa algoritmos
- `Planner::run_once` usa temperatura e top_k do BusinessLogic
- Seeds mudam a cada 15 minutos, garantindo reprodutibilidade

**C) Integração no Planner**
- `Planner` exige `Arc<BusinessLogic>` e aplica `plan_selection_bias()`
- Logs estruturados carregam seed, temperatura, top_k, índices
- `SelectedCandidate::into_decision` anexa `llm_action{}` à rationale

#### 🟡 FASE 2: LLM Integration (4 dias)

**A) Orquestrador e Circuit Breaker**
- `LlmHook` encapsula handler, budget_tokens, deadline e CircuitBreaker
- `CircuitBreaker` com janela deslizante (window_size=50, failure_threshold=0.10)
- `HttpLlmHandler` envia requests via `reqwest` com timeout

**B) Consumo no Planner**
- `Planner::apply_llm` cria `Vec<LlmInvocation>` e delega para orquestrador
- Quando `mode == Apply`, candidatos são reordenados deterministicamente
- Anotação `llm_action{}` adicionada às rationales finais

**C) Observabilidade e Testes**
- `tracing` (target `planner.llm`) registra source, reason, mode
- Testes assíncronos cobrem timeout, fallback e parsing
- Documentação operacional em `docs/LLM_HOOKS.md`

#### 🟠 FASE 3: Curator Vigilante & Token Bucket (4 dias)

**A) Configuração e Sinais**
- `CuratorVigilanteConfig` define thresholds e limites
- `evaluate_signals` calcula confiança agregada
- Sinais incluem duplicação de tags, desvio cromático, LUFS

**B) Token Bucket e Decisões**
- Quando `confidence ≥ threshold` e bucket libera crédito → `Apply`
- Ordem retornada respeita `max_reorder_distance`
- Logs diários em `logs/curator_vigilante/curator_vigilante_<data>.jsonl`

**C) Testes e Validação**
- `token_bucket_refills` garante refill horário
- `curator_detects_tag_duplication` verifica sinais estéticos
- Integração end-to-end Planner ↔ Curator

### 7.3 Próximos Passos (P4 em diante)

**Autopilot D+1:** orquestrar ajustes seguros com canary 20%, rollback automático, commits versionados do cartão.

**Incident learning:** transformar incidentes repetidos em patches sugeridos para o cartão.

**GitHub App auto-merge:** pipeline que assina PRs aprovados pelo Autopilot e libera merge supervisionado.

**Observability & Testing (P6/P7):** ampliar dashboards e suíte de testes integrados cobrindo LLM/Curator end-to-end.

### 7.4 Checklist de Qualidade

- ✅ Loader YAML validado com testes (bounds, ranges, parsing determinístico)
- ✅ Seleção Gumbel-Top-k coberta por testes unitários
- ✅ Circuit breaker e rerank testados com `tokio::test`
- ✅ Curator: token bucket + sinal de tags testados
- 🔜 Adicionar testes focados em `CuratorReview::order`
...
Com a integração do **Business Logic Engine**, o sistema agora possui:
- **Seleção determinística** via Gumbel-Top-K com auditabilidade completa
- **LLM integration** resiliente com circuit breakers e fallbacks
- **Curator Vigilante** para correções estéticas automáticas
- **Token bucket** para controle de intervenções
- **Hot reload** de configurações via CLI operacional

O resultado é uma televisão autônoma, transparente, sustentável e viva — que paga suas próprias contas, recompensa o próprio público pela atenção, e se adapta continuamente através de feedback computável e intervenções humanas calibradas.
